    ...
    const uploadSingle = async (file, index) => {
      try {
        ...
        if (data && data.success) {
          setUploadStatuses((s) => ({ ...s, [index]: { uploading: false, uploaded: true, filename: data.filename } }));
        } else {
          setUploadStatuses((s) => ({ ...s, [index]: { uploading: false, error: data.message || 'Upload failed' } }));
        }
      } catch (err) {
        setUploadStatuses((s) => ({ ...s, [index]: { uploading: false, error: err.message || 'Upload error' } }));
      }
    };
    ...
    const handleUploadAll = async () => {
      for (let i = 0; i < files.length; i++) {
        ...
        if (f.size > fileSizeLimit) {
          setUploadStatuses((s) => ({ ...s, [i]: { uploading: false, error: 'File too large' } }));
          continue;
        }
          ...
          <>
            <div className="file-list-header">
              ...
              <div style={{display: 'flex', gap: 8, alignItems: 'center'}}>
                <button type="button" className="uploadAllBtn" onClick={handleUploadAll}>Upload All</button>
                  ...
                  <div key={index} className={`file-item ${isOverLimit ? 'error' : ''}`}>
                    ...
                    <div className="file-item-details">
                      ...
                      <div className="upload-status">
                        {uploadStatuses[index]?.uploading && <span className="status uploading">Uploading…</span>}
                        {uploadStatuses[index]?.uploaded && <span className="status uploaded">Uploaded</span>}
                        {uploadStatuses[index]?.error && <span className="status error">{uploadStatuses[index].error}</span>}
                      </div>
                    </div>
                    <button 
                      type="button" 
